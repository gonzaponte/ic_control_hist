...
def pmaps_signal_histograms(files, signal, binlist, xrange = [None] * 5):
    ...
    '''
    S_e, S_total_e, S_t, S_total_t, S_t_weight_e = [[] for i in range(5)]
    for file in files:
        signal_pmaps = get_signal_pmaps([file], signal)

        _S_e = create_hist_df_simple(signal_pmaps.ene, binlist[0], xrange = xrange[0])
        _S_total_e = create_hist_df_simple(signal_pmaps.groupby('event').ene.sum(), binlist[1], xrange = xrange[1])
        _S_t = create_hist_df_simple(signal_pmaps.time_new, binlist[2], xrange = xrange[2])
        _S_total_t = create_hist_df_simple(signal_pmaps[['event', 'tot_time']].drop_duplicates().tot_time, binlist[3], xrange = xrange[3])
        _S_t_weight_e = create_hist_df_simple(signal_pmaps.time_new, binlist[4], weight=signal_pmaps.ene, xrange = xrange[4])

        S_e.append(_S_e)
        S_total_e.append(_S_total_e)
        S_t.append(_S_t)
        S_total_t.append(_S_total_t)
...
def pmaps_sensor_histograms(files, signal, sensor, binlist, xrange = [None] * 3):
    ...
    '''
    S_e_sensor, S_total_e_sensor, S_sensor_weight_e = [[] for i in range(3)]
    for file in files:
        sensor_pmaps = get_files_contents([file], 'PMAPS', signal + sensor)
        if sensor == 'Pmt':
            nsensor = 'npmt'
        if sensor == 'Si':
            nsensor = 'nsipm'
        _S_e_sensor = create_hist_df_simple(sensor_pmaps.ene, binlist[0], xrange = xrange[0])
        a = sensor_pmaps.groupby(['event', nsensor]).ene.sum().reset_index()
        _S_total_e_sensor = create_hist_df_simple(a.ene, binlist[1], xrange = xrange[1])
        _S_sensor_weight_e = create_hist_df_simple(sensor_pmaps[nsensor], binlist[2], xrange = (0, binlist[2]), weight=sensor_pmaps.ene)

        S_e_sensor.append(_S_e_sensor)
...
def pmaps_file_writer(files_path, out_path, city = 'hypathia', tag = '', xrange = [None] * 19):
    ...
    '''
    files = get_all_files(files_path, city)[:100]
